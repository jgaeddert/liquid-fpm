...
int main(int argc, char*argv[]) {
    ...
    fprintf(fid,"// scaling factor 1/Kp:  \n");
    fprintf(fid,"//   Kp = prod(k=0,infty){ sqrt(1 - 2^(-2*k)) } *\n");
    fprintf(fid,"//        prod(k=0,infty){ sqrt(1 - 2^(-2*i_k)) }\n");
    fprintf(fid,"// where\n");
    fprintf(fid,"//   i_0     = 4,\n");
    fprintf(fid,"//   i_(k+1) = 3*i_k + 1\n");
    fprintf(fid,"const %s_t %s_sinhcosh_cordic_kp_inv = 0x%.8x;\n\n", 
            qtype,qtype,
            qtype_float_to_fixed(kp_inv,intbits,fracbits));

    // generate table
    fprintf(fid,"// sinh|cosh cordic coefficients: A[k] = arctanh(2^-k)\n");
    fprintf(fid,"const %s_t %s_sinhcosh_cordic_Ak_tab[%u] = {\n", qtype,qtype,n);
    double inv_2_n   = 1.0;
    double Ak;
    unsigned int k;
    for (k=0; k<n; k++) {
