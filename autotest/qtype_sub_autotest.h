...
#ifndef LIQUIDFPM_QTYPE_SUB_AUTOTEST_H
#define LIQUIDFPM_QTYPE_SUB_AUTOTEST_H
...
void qtype_sub_autotest()
{
    float tol = Q(_fixed_to_float)(Q(_min));

    // basic tests
    Q(_test_sub)( 0.25f, 2.25f, tol);
    Q(_test_sub)( 0.25f,-2.25f, tol);

    // extremes
    Q(_t) x =  Q(_max)-1;
    Q(_t) y = -Q(_min);
    CONTEND_EQUALITY(Q(_sub)(x,y), Q(_max));
}

#endif // LIQUIDFPM_QTYPE_SUB_AUTOTEST_H
